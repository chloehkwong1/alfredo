...
function AnnotationBubble({ annotation, onDelete }: AnnotationBubbleProps) {
  return (
    <div className="ml-24 mr-4 my-1 flex items-start gap-2 px-3 py-1.5 rounded-md bg-accent-primary/10 border border-accent-primary/20">
      {/* Avatar */}
      <span className="flex-shrink-0 w-5 h-5 rounded-full bg-accent-primary text-text-on-accent text-micro font-semibold flex items-center justify-center mt-0.5">
        C
      </span>

      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className="text-caption text-text-primary">{annotation.text}</p>
